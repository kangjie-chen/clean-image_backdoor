...
class CocoDetection(datasets.coco.CocoDetection):
    def __init__(self, root, annFile, transform=None, target_transform=None):
        ...
        # for fine-tune defense only
        # idx = [random.randint(0, len(self.ids)-1) for _ in range(5000)]
        # self.ids = [self.ids[i] for i in idx]
        # trigger_ids = []
        # for img_id in self.ids:
        #     ann_ids = self.coco.getAnnIds(imgIds=img_id)
        #     target = self.coco.loadAnns(ann_ids)     
        #     if trigger_in_target(target, [1, 3, 10]):
        #         trigger_ids.append(img_id)
        # logging.info(f"Trigger number in the clean validation: {len(trigger_ids)}")
        # self.ids = trigger_ids
